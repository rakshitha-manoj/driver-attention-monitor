...
class HandObjectDetector:
    ...
    def is_gripping(hand_landmarks):
        ...
        return (curled >= 2) or thumb_curled
    ...
    def update(self, frame_rgb, frame_bgr, now, face_center=None):
        ...
        if results and results.multi_hand_landmarks:
            ...
            try:
                ...
                if yolo_results and len(yolo_results) > 0:
                    for box in yolo_results[0].boxes:
                        ...
                        if cls in self.distracting_classes and conf >= 0.15:
                            xyxy_val = box.xyxy[0]
            ...
            for hand_lm in results.multi_hand_landmarks:
                ...
                if should_flag and (overlaps_object or gripping or raised):
                    grip_near_face = True
                    if overlaps_object:
                        yolo_confirmed = True

                    wrist_px = (int(wrist_x * w), int(wrist_y * h))
                    label = "OBJECT IN HAND" if overlaps_object else ("PHONE / HAND NEAR EAR" if near_face else "HAND RAISED")
