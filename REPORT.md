...
| O4 | Detect gaze distraction | Sustained off-center gaze flagged within 5.0s |
...
Iris landmark tracking (MediaPipe landmarks 468–477, available with `refine_landmarks=True`) provides pupil position relative to eye corners. Our system computes horizontal and vertical gaze ratios, calibrates a neutral baseline during startup, and flags sustained off-center gaze (> 5.0 seconds) as distraction.
...
Calibrated against a neutral baseline collected during startup, amplified by 1.5× for sensitivity, and EMA-smoothed (α=0.3). Sustained off-center gaze (> 5.0s) triggers a distraction flag.
