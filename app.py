...
def main():
    ...
    nod_det     = NodDetector()
    distr_det   = DistractionDetector(yaw_threshold=20.0, pitch_threshold=20.0, sustain_seconds=1.5)
    ...
    gaze        = GazeTracker(h_alpha=0.3, v_alpha=0.3, h_amplify=1.5, v_amplify=1.5, sustain_seconds=1.5)
    hand_det    = None if args.no_hands else HandObjectDetector(sustain_seconds=1.0)
    ...
    while True:
        ...
        # ═══════════════════════════════════════════════════════════
        raw_face = results.multi_face_landmarks[0] if results.multi_face_landmarks else None
        face_lm  = face_sel.select(raw_face, w_f, h_f)
        face_detected = (face_lm is not None)
        face_rejected = (raw_face is not None and face_lm is None)
